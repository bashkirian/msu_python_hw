...
def leastSquares(f, deg):
    ...
    Return: the coefficients of the approximating polynomial
            in descending order.'''
    x = np.array([i[0] for i in f])
    y = np.array([i[1] for i in f])
    print(deg)
    a = np.vander(x, deg + 1)
...
def main():
    ...
    def plotFunction():
        ...
        if plotID >= 0:
            ...
            plotID = (-1)       
        nodesX = [p[0] for p in points]
        y = np.polyval(coeffs, xMin())
        # y = newtonPolValue(n, nodesX, coeffs, xMin())
        t = R2Point(xMin(), y)
        while t.x <= xMax():
            path.append(map(t))
            t.x += dx
            # t.y = f(t.x)
            t.y = np.polyval(coeffs, t.x)
        plotID = drawArea.create_line(path, fill="blue", width=2)
        funcDrawn = True
        
    def onMouseRelease(e):
        nonlocal catchedPoint, coeffs
        p = (e.x, e.y)
        t = invmap(p)
        if catchedPoint < 0:
            points.append(t)
            ...
            if funcDrawn:
                coeffs = leastSquares(points, degree)
            redraw()
            catchedPoint = (-1)
